...
using namespace std;
Group :: Group() { transformation = Transformation(); pVbos = vector<GLuint>();nVbos = vector<GLuint>();tVbos = vector<GLuint>();textures = vector<GLuint>() ; maxX = 0.0f;maxZ = 0.0f;numAst = 0;}
Group::Group(Transformation t, vector<GLuint> pvb, vector<GLuint> nvb, vector<GLuint> tvb, vector<GLuint> texture, float maX, float maZ, int nAst)
...
void Group::setTextures(vector<GLuint> t){this->textures = t;}
...
vector<GLuint> Group :: getTextures(){return this->textures;}
